...
public class JWTService {
  ...
  public String generateAccessToken(UserDetails user) throws TokenGenerationException {
    try {
      ...
      log.error(
          "Failed to generate access token for user: {}, error: {}",
          user.getUsername(),
          e.getMessage());
  ...
  public String generateRefreshToken(UserDetails user) throws TokenGenerationException {
    try {
      ...
      log.error(
          "Failed to generate refresh token for user: {}, error: {}",
          user.getUsername(),
          e.getMessage());
  ...
  public String getUsernameFromToken(String token) throws InvalidTokenException {
    try {
      ...
      log.error("Failed to extract username from token: {}, error: {}", token, e.getMessage());
